...
const { brandSchema } = require('../youtubeSchemas');
const fs = require('fs');
const path = require('path');

const PROMPT_TEMPLATE = fs.readFileSync(path.join(__dirname, '../prompts/brandPrompt.js'), 'utf-8');

async function runBrandAgent(project, scriptOutput, thumbnailConcept, finalTitle, seoPackage, editorPlan, feedback = "") {
    ...
    for (let attempts = 1; attempts <= 3; attempts++) {
        try {
            console.log(`[Brand Agent] Attempt ${attempts}...`);
            const passThroughSchema = { parse: (data) => data };
            let rawData = await generate({ agentId: 13, sysPrompt: PROMPT_TEMPLATE, userPrompt: prompt, schema: passThroughSchema, isScript: false });

            let validated;
            try {
                validated = brandSchema.parse(rawData);
            } catch (zodErr) {
                if (zodErr.errors) {
            ...
            lastError = e;
            console.warn(`Brand attempt ${attempts} failed:`, e.message);
            if (attempts === 1) {
